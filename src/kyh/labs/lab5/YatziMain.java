...
public class YatziMain {
    ...
    private static void gameTurn(int i){
        ...
        if (rollDiceAndCheckIfYatsi()){                     // Rullar tärningarna och returnerar sant om det blir Yatsi.
    ...
    private static boolean rollDiceAndCheckIfYatsi(){
        int lastFaceValue = 0;                                      // Sparar föregående värde för jämförelse.
        boolean yatzi = true;                                       // Sätter flaggan för yatsi till sann.
        for (int i = 0; i < 5; i++) {
            faceValue = new Die().faceValue; // Konstruerar en ny tärning, den får sitt slumpvärde i konstruktorn (Die.java).
